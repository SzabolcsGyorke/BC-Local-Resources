...
It will register a few web services for access - make sure that they are accesible for the Client machine.
...
### Connection details
oData URL: base url for the BC services including the compnay name._ Example for MS Cloud: https://api.businesscentral.dynamics.com/v2.0/6e9fbeef-85d9-4bcb-8ec4-xxxxxxxxxb9/Production/ODataV4/Company('CRONUS%20UK%20Ltd.')/_
...
- **Cleint Secret**: cleint secret form azure
...
- **Scope**: defult scope for BC _Example: https://api.businesscentral.dynamics.com/.default_

![image](https://user-images.githubusercontent.com/64136814/218325198-9943ab76-8edd-49a9-910b-58c5693853df.png)

**Basic**
- **Username**: BC username
- **Password**: BC passord or API key.
...
- **Register Instance**: registr the service and local printers for the use of BC
- **Update Hearbeat**: updates the BC service record
...
- **Token Information**: diplay the current token and expiry time (oAuth only)

**Once you tested all the requered components use the _Upload Configuration_ function to store it in BC and for furter use to configure the Windows Service.**
...
### File Services
Display or add registered folder used by BC to syncronize.
...
- **Syncronize files**: upload and download files from **all** the folders
...
- **Functions**: select whic function or functions you want to run
- **Start**: starts the timer - will turn green while the timer is running
- **Clear Log**: deletes the log

## Windows Service 
The windows service runs in the backrouund and syncronises with BC in a set time interval.
...
If your run multiple services on the same machine the Source collumn will help to identify the instance.
